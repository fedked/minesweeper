...
class Tabuleiro(val qtdeLinhas: Int, val qtdeColunas: Int, private val qtdeMinas: Int) {
    ...
    private fun gerarCampos() {
        for (linha in 0 until qtdeLinhas) {
            ...
            for (coluna in 0 until qtdeColunas) {
                ...
                novoCampo.onEvento(this::verificarDerrotaVitoria)
    ...
    private fun verificarDerrotaVitoria(campo: Campo, evento: CampoEvento) {
    ...
    fun forEachCampo(callbacks: (Campo) -> Unit) {
        campos.forEach { linha -> linha.forEach(callbacks) }
    }
